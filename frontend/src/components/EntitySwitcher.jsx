 ...
 */
export default function EntitySwitcher({ entities = [], value = "all", onChange, canSwitch = true, role = "" }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);

  const ROLE_LABEL = { admin: "Admin", manager: "Manager", sales: "Sales", warehouse: "Gudang" };
  const roleLabel = ROLE_LABEL[role] || (role ? role.charAt(0).toUpperCase() + role.slice(1) : "");
  ...
  if (!canSwitch) {
    ...
    return (
      <div className="entity-switcher" data-testid="entity-switcher-locked">
        <div className="entity-switcher-trigger" title={`Anda di ${lockLabel}${roleLabel ? ` sebagai ${roleLabel}` : ""} (terkunci)`} style={{ cursor: "default" }}>
          <Building2 size={14} />
          <span className="entity-switcher-label">{lockLabel}</span>
          {roleLabel && <span className="entity-switcher-role" data-testid="entity-role-tag">· {roleLabel}</span>}
        </div>
      </div>
    );
  }
