...
export default function MobileCart({
  ...
  async function placeOrder() {
    ...
    setSubmitting(true);
    try {
      const ok = await onSubmitOrder({ payment_term_code: termCode, allow_backorder: allowBackorder });
      ...
      {step === "cart" ? (
        ...
        <>
          ...
          <div className="m-card space-y-3 p-4">
            ...
            <div data-testid="mobile-team-inherit-note" className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] px-2.5 py-2 text-[11px] text-[#6B6B73]">
              Tim sales & split insentif mengikuti pengaturan <b>customer</b> (atur di Manajemen Pelanggan).
            </div>
          </div>
        </>
      )}
