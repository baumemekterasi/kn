    ...
    <div data-testid="special-orders-view" className="view-container">
      ...
      <div data-testid="special-order-stats" className="grid grid-cols-2 gap-2 sm:grid-cols-5" style={{ marginBottom: 12 }}>
        {[
          ...
          { label: "Draft", value: soCnt("draft"), icon: Package },
          { label: "Menunggu Approval", value: soCnt("pending_approval"), icon: Clock },
      ...
      {loading ? (
        ...
        <div className="empty-state" data-testid="special-order-empty">
          ...
          <p className="font-semibold">Belum ada special order {statusFilter ? `dengan status ${statusTabs.find(t => t.key === statusFilter)?.label}` : ""}.</p>
