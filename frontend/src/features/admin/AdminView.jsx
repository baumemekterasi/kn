...
export default function AdminView({
  ...
  const tabs = [
    ["entities", "Entities"], ["products", "Product"], ["categories", "Kategori"], ["customers", "Customer"], ["warehouses", "Warehouse"], ["uoms", "UOM"], ["settings", "Pengaturan"], ["integrations", "Integrasi AI"], ["templates", "Templates"], ["permissions", "Permissions"], ["audit", "Audit"], ["users", "Users"],
  ];
  ...
  return (
    <div data-testid="admin-view">
      ...
      {tab !== "settings" && tab !== "categories" && tab !== "integrations" && (
      <section className="grid gap-3 lg:grid-cols-[360px_1fr]">
        ...
        <div className="section-card">
          ...
          {tab === "users" && <div className="grid gap-2">
            {[["name", "Nama user"], ["email", "Email"], ["password", "Password"]].map(([key, ph]) => <input key={key} data-testid={`admin-user-${key}-input`} className="field" placeholder={ph} value={userForm[key]} onChange={(e) => setUserForm({ ...userForm, [key]: e.target.value })} />)}
            <KNSelect data-testid="admin-user-role-input" className="field" value={userForm.role} placeholder="Role"
              onValueChange={(v) => setUserForm({ ...userForm, role: v })}
              options={[{ value: "sales", label: "Sales" }, { value: "manager", label: "Manager" }, { value: "warehouse", label: "Gudang" }, { value: "admin", label: "Admin" }]} />
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wide text-[#8E8E93]">Entitas Utama (home)</label>
              <KNSelect data-testid="admin-user-home_entity_id-input" className="field" value={userForm.home_entity_id || ""} placeholder="Pilih entitas utama"
                onValueChange={(v) => setUserForm((f) => ({ ...f, home_entity_id: v, allowed_entity_ids: Array.from(new Set([...(f.allowed_entity_ids || []), v])) }))}
                options={(entities || []).map((e) => ({ value: e.id, label: e.legal_name || e.short_name || e.id }))} />
            </div>
            <div data-testid="admin-user-allowed-entities" className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] p-2">
              <label className="text-[10px] font-bold uppercase tracking-wide text-[#8E8E93]">Entitas Diizinkan (multi-entitas)</label>
              <div className="mt-1 flex flex-wrap gap-1.5">
                {(entities || []).map((e) => {
                  const on = (userForm.allowed_entity_ids || []).includes(e.id);
                  const isHome = userForm.home_entity_id === e.id;
                  return (
                    <button key={e.id} type="button" data-testid={`admin-user-allowed-${e.id}`}
                      onClick={() => setUserForm((f) => {
                        const cur = new Set(f.allowed_entity_ids || []);
                        if (cur.has(e.id)) { if (!isHome) cur.delete(e.id); } else cur.add(e.id);
                        return { ...f, allowed_entity_ids: Array.from(cur) };
                      })}
                      className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${on ? "border-[#0058CC] bg-[#EFF4FF] text-[#0058CC]" : "border-[#E5E5EA] bg-white text-[#6B6B73]"}`}>
                      {e.short_name || e.legal_name || e.id}{isHome ? " ★" : ""}
                    </button>
                  );
                })}
              </div>
              <p className="mt-1 text-[10px] text-[#9A9BA3]">Sales bisa di-assign &gt;1 entitas → bisa berpindah konteks lewat switcher. Entitas utama (★) selalu termasuk.</p>
            </div>
