...
export function OrderDetailPanel({
  ...
  // F5/RBAC — hanya admin/manager yang boleh approve/confirm; sales lihat status saja.
      ...
      <div className="section-body space-y-3">
        ...
        <div data-testid="order-pricing-breakdown" className="rounded-md border border-[#EFF0F2] bg-[#FAFBFC] p-2.5 space-y-1 text-[11.5px]">
          ...
          {sel.needs_tax_invoice && (
            <div data-testid="order-needs-faktur-badge" className="mt-1 flex items-center gap-1 rounded bg-[#EFF4FF] px-2 py-1 text-[10px] font-semibold text-[#0058CC]">
              <FileText size={11} /> Customer minta Faktur Pajak
            </div>
          )}
        ...
        {sel.fulfillment_method === "ambil" && (
          <div data-testid="order-pickup-badge" className="flex items-center gap-2 rounded-md border border-[#CDE3FF] bg-[#EFF4FF] px-2.5 py-1.5 text-[11px] text-[#0058CC]">
            ...
            <span>Ambil di Gudang{sel.pickup_date ? ` · ${sel.pickup_date}` : ""} — picking di-hold sampai tanggal ambil</span>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {sel.status === "waiting_approval" && isApprover && (
          ...
          {sel.status === "approved" && isApprover && (
