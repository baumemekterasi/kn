        ...
        <div className="section-body">
          ...
          {scopeEntityId && (
            <div className="mb-3 rounded-md border border-[#E6E0F2] bg-[#FAF8FE] px-3 py-2 text-[11px] text-[#6B219A]" data-testid="coa-scope-note">
              <b>Mode Per-PT:</b> akun tanpa <i>badge</i> = warisan template global (dipakai apa adanya). Akun dengan badge <b>override</b> = khusus PT ini. Posting jurnal tetap merujuk template global.
            </div>
          )}
                    ...
                    <table className="w-full text-[12px]">
                      <tbody>
                        {grouped[t].map((a) => {
                          ...
                          return (
                          <tr key={a.code + "|" + (a.entity_id || "g")} data-testid={`coa-row-${a.code}`} className={`border-b border-[#F5F5F7] last:border-0 ${a.is_postable ? "" : "bg-[#FAFBFC]"}`}>
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-[#EFF0F2]">
          ...
          <h3 className="font-bold text-[14px]">
            Edit Akun <span className="font-mono tabular-nums text-[#6B6B73]">{acc.code}</span>
          </h3>
