...
class UserCreate(BaseModel):
    ...
    home_entity_id: str = ""              # F6 — entitas kerja/payroll user
    allowed_entity_ids: List[str] = []    # F6 — entitas yang boleh dioperasikan (multi-entitas)
...
class RollLineIn(BaseModel):
    ...
    roll_id: str
    take_qty: float = 0                  # 0 → diisi backend = length_remaining (roll utuh)
...
class SoSpecialPriceRequest(BaseModel):
    ...
    requested_price: float                 # harga khusus per unit (jual)
