...
async def seed_sales_extras_foundation() -> None:
    ...
    if await db.special_orders.count_documents({"entity_id": entity_id}) == 0:
        ...
        if cust:
            ...
            samples = [
                {"desc": "Kain jacquard custom motif logo perusahaan", "qty": 500, "unit": "meter", "price": 85000, "status": "draft"},
                {"desc": "Sutra dobby warna Pantone khusus (indent 6 minggu)", "qty": 300, "unit": "meter", "price": 145000, "status": "pending_approval"},
                {"desc": "Katun premium bordir custom seragam", "qty": 1200, "unit": "meter", "price": 42000, "status": "in_production"},
            ]
            for s in samples:
                total = s["qty"] * s["price"]
    ...
    if await db.sales_returns.count_documents({"entity_id": entity_id}) == 0:
        ...
        if order:
            ...
            base_item = {"product_id": it.get("product_id", ""), "product_name": it.get("product_name", "Produk"),
                         "quantity_returned": 10, "unit": it.get("unit", "meter"),
