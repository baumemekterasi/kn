...
from core_utils import new_id, now_iso, safe_doc, strip_cost_fields
...
async def stock_breakdown(product_id: str, request: Request) -> Dict[str, Any]:
    # S-10 — role penentu redaksi HPP (endpoint historis tanpa guard; default teredaksi)
    role = None
    try:
        from dependencies import current_user
        role = (await current_user(request)).get("role")
    except Exception:
        role = None
