...
async def create_price_approval(payload: PriceApprovalCreate, request: Request) -> Dict[str, Any]:
    ...
    scope = "order" if (payload.scope or "").strip().lower() == "order" else "standing"
    doc = {
        "id": new_id("pra"),
        "entity_id": entity_id,
        ...
        "valid_from": now_iso(),
        "valid_until": "" if scope == "order" else _norm_until(payload.valid_until),
        "status": status,
        "scope": scope,
        "so_id": (payload.so_id or "").strip(),
        "is_override": bool(payload.override),
        "attachments": [],
    ...
    await audit(user.get("name", ""), "price_approval_created", "price_approval", doc["id"], {
        ...
        "scope": scope, "override": bool(payload.override),
    })
    ...
    for old in superseded_docs:
        ...
        await create_notification(
            ...
            body=(f"Aturan Anda untuk {cust} · {prod} digantikan oleh "
                  ...
                  f"Harga: Rp {old_price:,.0f}/{unit} {arrow} Rp {new_price:,.0f}/{unit} "
                  f"(Δ {delta_pct:+.1f}%)."),
            severity="warning",
