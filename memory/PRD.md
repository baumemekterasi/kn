...
**Last Updated:** 3 Jul 2026 (M-3: CoA per-PT Override + Inter-Company Transfer JE SELESAI)  
**Status:** Active Development

> ### 📌 SESI 3 JUL 2026 (M-3) — CoA per-PT Override + Inter-Company Transfer JE
> **CoA per-PT Override (non-breaking):** `gl_accounts` sekarang punya field `entity_id` (nullable). `entity_id=None` = template global (SHARED). `entity_id=<ent_id>` = akun khusus PT: bisa akun baru (yg tidak ada di template) atau override akun template dengan `code` sama. Endpoint `/api/gl/accounts` (GET/POST/PATCH/DELETE) menerima query param `entity_id` optional. Effective view (GET dengan `entity_id`) memenangkan override PT per-code. Isolasi lintas-PT valid (KSC override tidak muncul di KANDA). **Posting JE tetap resolve by-code global** (aman: tidak breaks sales/COGS/cash/manual JE).
> **Inter-Company Transfer JE (M-3):** `execute_ownership_transfer` di router transfers sekarang otomatis memanggil `gl_service.post_intercompany_transfer()` yang mem-post 2 JE seimbang (at-cost):
> - **Buku SOURCE** (B): `Dr 1-1250 IC-AR / Cr 1-1300 Persediaan`
> - **Buku DEST** (E): `Dr 1-1300 Persediaan / Cr 2-1250 IC-AP`
> - Nilai = qty × WAC dari entitas source (konservatif: no IC profit sampai barang dijual eksternal — hindari double-count saat konsolidasi).
> - Idempotent via `source_type='inter_company_transfer'` + `source_id=<transfer_id>:src|:dst`. Kedua JE dilink via `intercompany_pair_id='ict_<transfer_id>'` + `intercompany_counterpart_entity_id` untuk konsolidasi/elimination.
> - 2 akun baru di DEFAULT_COA: `1-1250` (IC-AR), `2-1250` (IC-AP).
> **Verifikasi:** testing_agent_v3 iter_111 — **Backend 17/17 PASS** (7 CoA per-PT + 5 IC JE + 5 regression). Trial balance kedua PT tetap seimbang. Intra-entity/rejected transfer tidak trigger IC JE. Idempotent verified.
> **Gates:** verify_data_integrity **119 PASS / 0 FAIL**, verify_api_contract **0 ERROR**, audit_endpoint_sweep **0 5xx**.
>
> ### 📌 SESI 3 JUL 2026 — Add Entity Auto-Refresh Fix
> Bug fix: `build_entity_context()` (dipakai `/auth/me`, login response) sekarang resolve DINAMIS untuk role `admin/manager` — entitas baru yang di-provision langsung muncul di Entity Switcher tanpa perlu logout-login. Sales/warehouse tetap silo per record user.
>
> ### 📌 SESI 3 JUL 2026 (Lanjutan) — Notifikasi Push saat Aturan Harga Di-Supersede
> **Enhancement:** `routers/price_approvals.py` `approve_price_approval` sekarang mengirim notifikasi ke setiap `requested_by` (sales owner) yang aturannya di-supersede. Notif memuat: customer, product, nama approver, harga lama → baru (Rp), Δ%, arrow ↑↓, dan deep-link (`action_type=price_approval_view`, `action_id=<new_id>`). Dedupe via `ref="pra_superseded:<old_id>"`. Severity `warning`, `recipient_role='sales'` + `recipient_user=<owner>`. Order-scope tidak mem-picu notifikasi (juga tidak mem-picu supersede).
> **Verifikasi:** testing_agent_v3 iter_110 — **Backend 28/28 PASS (+1 legit skip)**, 0 critical / 0 minor. E2E manual: sales user menerima 1 notifikasi tepat setelah approval mereka digantikan admin.
> **Tech-debt refresh:** `sales_orders.py` = 793 baris (di bawah limit 800 ✅), `CheckoutDrawer.jsx` = 496 baris (di bawah limit 500 ✅). Refactor sudah dilakukan di commit sebelumnya — validate_compliance sekarang **94 PASS / 0 FAIL / 58 WARN**.
>
> ### 📌 SESI 3 JUL 2026 — Price Approval: Supersede-on-Approve
> **Konteks:** Repo di-restore dari `github.com/indonesiamembungkam/kn`, `.env` dipertahankan (MONGO_URL/DB_NAME/REACT_APP_BACKEND_URL). Backend jalan, `scripts/seed_reset.sh` LULUS, semua gate hijau (verify_contract, verify_api_contract 0 ERROR, verify_data_integrity 119 PASS / 0 FAIL, audit_endpoint_sweep 0 5xx).
> **Fitur:** Override/replace pada `POST /api/price-approvals/{id}/approve` (`routers/price_approvals.py` lines 324-337) — aturan STANDING yang baru disetujui otomatis *supersede* aturan approved lain untuk tuple (entity_id, customer_id, product_id) yang sama. Yang terbaru menang; aturan lama di-set `status='superseded'`, `superseded_by=<new_id>`, `superseded_at=<now_iso>`. Approval `scope='order'` DIKECUALIKAN (tidak men-supersede, tidak di-supersede). Audit log `price_approval_approved` menyertakan `superseded_count`.
> **Verifikasi:** testing_agent_v3 iter_109 — **Backend 16/16 PASS**, 0 critical / 0 minor. E2E manual juga confirm: id1(approved) → id2 approve → id1 jadi superseded, superseded_by=id2.
> **Tech-debt catatan (dari review):** pertimbangkan index `price_approvals(entity_id, customer_id, product_id, status)` bila koleksi tumbuh besar; endpoint `/api/audit-logs` belum paginated (limit 500 desc).
...
### v1.14 — 2 Jul 2026 (GELOMBANG 3 — S-10 REDAKSI HPP/BIAYA UTK ROLE SALES) ✅
- **Helper terpusat** `core_utils.strip_cost_fields(data, role)` — rekursif hapus `COST_FIELDS` (unit_cost, base_unit_cost, harga_pokok, landed_cost_total/refs, cogs_amount, cogs, wac, margin, margin_pct) dari respons bila role BUKAN admin/manager (sales/warehouse/anonim teredaksi). Data DB tetap utuh (snapshot cost-at-sale utk margin insentif manager).
- **Diterapkan pada endpoint yang bisa diakses sales**: `sales_orders.py` (list/get/create/update/submit/deliver/release/cancel/frequent-products), `products.py` (GET /products — juga anonim; GET /products/{id}/stock-breakdown — endpoint historis tanpa guard kini teredaksi default), `inventory.py` (rolls, rolls/available, movements), `sales_returns.py` (credit-notes list/detail — cogs_amount).
- **Verified (self-test curl matrix)**: sales → 0 kemunculan unit_cost/harga_pokok/cogs_amount di orders/products/rolls/credit-notes/stock-breakdown; admin → semua field utuh (19/11/27 kemunculan); create SO sebagai sales sukses (respons tanpa unit_cost, DB tetap menyimpan 122100); UI Sales Home + POS Sales Portal normal (screenshot).

### v1.13 — 2 Jul 2026 (GELOMBANG 3 AUDIT — P0: AUTH + PPN 12% + G-1) ✅
- **SEC-1/SEC-2 Auth rewrite** (`core_utils.py`, `dependencies.py`, `routers/auth.py`): bcrypt (migrasi transparan SHA256→bcrypt rehash saat login sukses), session token 256-bit (`secrets.token_urlsafe(32)`), TTL 24 jam via Mongo TTL index `sessions.expires_at` + sliding renewal (< setengah TTL), HttpOnly cookie `session_token` (fallback Bearer tetap didukung — FE tanpa perubahan flow), lockout brute-force 5 gagal per IP+email → 429 15 menit (`login_attempts`). FE: `axios.defaults.withCredentials=true`.
- **F-10 PPN 12% / DPP Nilai Lain 11/12 (Coretax, PMK 131/2024)** — configurable per entitas (override scope `system_settings`): `tax.ppn_rate=12` + `tax.dpp_nilai_lain=true` → DPP = 11/12 × harga jual, PPN efektif 11% (rupiah identik rezim lama). Migrasi idempotent `migrate_tax_regime()` saat startup. Order/PO menyimpan `dpp_nilai_lain` + `effective_rate`. **GL**: revenue & GRIR = harga jual/beli neto (grand − PPN), BUKAN DPP nilai lain. Faktur Pajak: kode transaksi default **04** bila nilai lain (schema `TaxInvoiceCreate.kode_transaksi` default None), snapshot + HTML "DPP — Nilai Lain 11/12". FE: toggle di SettingsPanel (`settings-dpp-nilai-lain`) & TaxConfigPanel (`tax-config-dpp-nilai-lain`), preview POS/PO/pricing.js mirror efektif 11%, label "PPN 12% (DPP 11/12)".
- **BONUS BUG FIX**: PPN retur penjualan (`return_service.py` L89) sebelumnya `net × ppn_rate` tanpa /100 (11× lipat) → kini rasio efektif dari order asal (`ppn_amount/(grand−ppn)`), sekaligus dukung nilai lain.
- **G-1**: 7 koleksi (hr_shifts, hr_geofences, hr_attendance, hr_devices, hr_field_tracks, hr_visits, tax_pph_records) didaftarkan ke `ENTITY_REGISTRY.md` → gate L0 `verify_data_integrity` PASS (53 koleksi konsisten).
- **Verified**: testing_agent iteration_98 — backend 13/13 pytest PASS (`backend/tests/test_gelombang3_auth_ppn12.py`), frontend PASS (settings/tax toggle, login/logout/relogin, navigasi hub). E2E: SO→payment→JE balanced (Cr Pendapatan = grand−ppn), Faktur kode 04, PO PPN efektif 11%, non-PKP ent_kanda tetap 0.
- **Sisa Gelombang 3 (P1/P2, belum dikerjakan)**: S-10 strip unit_cost utk role sales · F-7 unifikasi costing · F-8 workflow suspense · F-9 tutup buku tahunan · badge count Pusat Persetujuan/Eskalasi.

### v1.12 — 2 Jul 2026 (RESTRUKTURISASI IA — OPSI A PENUH) ✅
- **Navigasi hub-and-tab**: admin 62 → **±23 menu** (Penjualan 10→4, Pembelian 12→4, Gudang 9→2, Keuangan 8→6, SDM 12→4, Analitik 4→1, Pengaturan 2→1); pola: menu = proses bisnis, variasi = tab bar sekunder (`HubTabs`), komponen view TIDAK berubah, deep-link lama tetap hidup via `resolveActiveNavId`.
- **7 duplikasi domain dilebur**: Kas satu pintu (Kas & Bank), Pajak satu pintu di Keuangan (Faktur Keluaran/Masukan/PPh & Rekap — keputusan owner), Approval satu pintu (Pusat Persetujuan + tab Harga/Pembelian), Stok & ATP, Transfer (antar-gudang+antar-entitas dalam hub WMS), Kunjungan→CRM, Approval Rules→Pengaturan.
- **Command Palette (Ctrl+K)** — cari & lompat ke menu mana pun (role-aware, grouped) + tombol "Cari menu…" di sidebar.
- **Favorit/Pin** — bintang di item sidebar, max 5, persist localStorage per user, seksi FAVORIT di atas.
- **Role hygiene**: warehouse tak lagi melihat Tagihan Supplier/Landed Cost/Pajak; landing warehouse→Operasi WMS, manager→Analytics Hub.
- Bersih-bersih: 9 PAGE_META `cs-*` mati dihapus, `ConsolidationDashboard.jsx` (dead) dihapus, grup 1-item "Dokumen & Print" jadi standalone; gate `check_nav_map` diselaraskan (PASS).
- File: `navigationConfig.js` (rewrite), `HubTabs.jsx` + `CommandPalette.jsx` (baru), `CoreWidgets.jsx` (favorit+search), `App.js`.
- Testing: testing agent iteration_98 — **19/19 hub & 55/55 tab hijau, 0 console error**; self-test: palette select, favorit persist, role sales/warehouse/manager, regresi POS/GL-recon/WMS/approval inbox — semua PASS. Gate: check_nav_map PASS, verify_contract OK.

### v1.11 — 2 Jul 2026 (GELOMBANG 2 AUDIT — OPERASIONAL) ✅
- **S-4** Cancel SO kini membatalkan task picking outbound aktif (antrean gudang bersih).
- **W-1** `GET /outbound/tasks` ter-scope entitas (X-Entity-Id / ?entity_id, `all` utk lintas-PT); release/scan/escalate/resolve/dispatch ber-guard `assert_entity_access`.
- **S-3** Order Pengambilan divalidasi di API: tanpa `pickup_date` atau tanggal lampau → 400.
- **S-5** Lookup produk by-id (hapus cap `to_list(100)`/`to_list(2000)`) di create_order & preview.
- **S-6** Resolve eskalasi dgn `adjusted_qty` kini sinkron penuh: `release_order_rolls_partial` (roll dilepas utuh/dipecah), allocation + item (`base_quantity`, `quantity`, `reserved_qty`) + repricing (total/DPP/PPN/grand/approval_amount) + timeline `qty_adjusted`; guard anti-naik, anti < picked, blok bila sudah ada pembayaran.
- **S-9** `_split_roll` atomik (find_one_and_update + guard length; anti double-reserve konkuren).
- **W-2 (validasi roll scan-pick) DITUNDA** — owner akan implementasi RFID.
- Testing: uji empiris per-fix via API (semua PASS), skenario S-6 e2e sampai dispatch (SO shipped), gate invarian 64 PASS (hanya FAIL G-1 lama), endpoint sweep 144 OK / 0×5xx, smoke UI outbound.

### v1.10 — 1 Jul 2026 (GELOMBANG 1 AUDIT — INTEGRITAS AKUNTANSI GL) ✅
- **F-1** Reversal jurnal otomatis saat SO cancel/expire (`gl_service.reverse_order_journals`; JE asli diberi flag `reversed`, net akun = 0).
- **F-2** Pengakuan pendapatan dibatasi: hanya order `shipped/partially_shipped/done` ATAU sudah terbayar (`_revenue_eligible`); tanggal JE = tanggal event pengiriman/pembayaran (`_revenue_date`).
- **F-3** GR→GL otomatis saat inbound complete (Dr 1-1300 Persediaan / Cr 2-1150 GR-IR baru); endpoint `GET /gl/inventory-reconciliation` + `POST /gl/inventory-opening-balance` (idempotent harian); tab baru "Rekonsiliasi Persediaan" di GL UI. True-up saldo awal Rp 532,9jt terposting → GL = subledger (selisih 0).
- **F-4** `simulate-payment` order AR kini membuat cash_transaction settlement (Dr Kas / Cr Piutang, pola AR Receipt; order tunai tidak dobel).
- **F-5** Vendor Bill posted → JE Dr GR-IR + Dr 1-1500 PPN Masukan / Cr 2-1100 Hutang; `gl/sync` juga backfill vendor bills; AP tidak bisa negatif lagi.
- **F-6** `as_of` tanggal-saja di trial balance & account ledger kini day-end (konsisten dgn laporan keuangan).
- COA baru: 2-1150 GR/IR, 3-2900 Ekuitas Saldo Awal. Testing: uji manual (F-1/2/3/4/6 PASS) + testing agent iteration_97 (17/18 pytest, 0 isu critical; 1 flake test-side). Gate seed 119 PASS.
- Sisa minor → Gelombang berikut: reversal proporsional partial-shipment, GR-IR reversal utk retur beli/QC reject, refund kas order terbayar yang dibatalkan.

### v1.9 — 1 Jul 2026 (AUDIT FORENSIK MENYELURUH)
- **Audit forensik seluruh sistem** (static + empiris via API pada clean seed) → `memory/FORENSIC_AUDIT_2026-06.md`.
- Terverifikasi BENAR: penghapusan diskon manual (backend memaksa 0), pickup order hold (`scheduled` + auto/manual release), Returns/Special Orders stats + seed demo.
- Temuan Critical (belum diperbaiki — menunggu review owner): GL tanpa reversal saat cancel [terbukti], pengakuan pendapatan prematur (order `reserved` ikut diposting), GL Persediaan vs subledger selisih Rp 533,7jt, simulate-payment tanpa jurnal settlement, Vendor Bill tanpa posting GL.
- Temuan High/Medium: cancel SO confirmed meninggalkan task picking aktif [terbukti], outbound tasks bocor lintas-entitas [terbukti], pickup tanpa tanggal lolos API, cap 100 produk di create_order, as_of trial balance memotong hari yang sama [terbukti], dll — lihat dokumen audit §11 untuk urutan prioritas 3 gelombang.
- Status: MENUNGGU KEPUTUSAN OWNER untuk eksekusi perbaikan Gelombang 1 (integritas akuntansi).
